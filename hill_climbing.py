...
# if true will output the time each problem took to complete.
debug = False
...
class NQueensProblem:
    ...
    def actions(self, state: tuple) -> list:
        ...
        # create the actions output.
        actions = []
        
        # iterate through each possible neighbour which will be N-1 * N states
        for i in range(self.N):
            duplicateState = list(state)
            for j in range(self.N):
                if (state[i] == j):
                    continue

                # change the value in the duplicate state to j
                duplicateState[i] = j

                # create a copy of the state to add to the actions
                toAdd = tuple(duplicateState.copy())

                # add the state to the actions list
...
def hill_climbing_instrumented(problem):
    ...
    while True:
        ...
        # update this each time becuase we only increase the state
        ...
        # if the neighbour is not strictly greater than the current value.
        if neighbour.value() <= current.value():
            break
    ...
    if debug:
        print("time taken for hill climbing instrumented: %.10f seconds" % (end_time - start_time))
...
def hill_climbing_sideways(problem, max_sideways_moves):
    ...
    while True:
        ...
        # if the neighbour is not strictly greater than the current value.
        if sideways_moves < max_sideways_moves and neighbour.value() == current.value():

            # add a sideways move when it can move sideways
            sideways_moves += 1
    ...
    if debug:
        print("time taken for hill climbing sideways: %.10f seconds" % (end_time - start_time))
...
def hill_climbing_random_restart(problem, max_restarts):
    ...
    solved = False
    restarts = 0
    best = Node(problem=problem, state=problem.initial)
    ...
    while True:
        ...
        # only change best_state if it is better
        if not current:
            break
        ...
        # if the neighbour is not strictly greater than the current value.
        if restarts < max_restarts and neighbour.value() == current.value():

            # add a random restart and move to a new state
            restarts += 1
    ...
    if debug:
        print("time taken for hill climbing random restarts: %.10f seconds" % (end_time - start_time))
...
def test_time():
    nqueens8 = NQueensProblem(10)
    nqueens9 = NQueensProblem(30)
    nqueens10 = NQueensProblem(50)

    # variables which count how any times each method passed
    hci_pass8 = 0
    hcs_pass8 = 0
    hcrr_pass8 = 0
    hci_pass9 = 0
    ...
    # run a test for each of the seeds
    ...
    for seed in range(test_range):
        ...
        temp = hill_climbing_instrumented(nqueens8)
        if temp.get("solved"):
            hci_pass8 += 1

        temp = hill_climbing_sideways(nqueens8, maximum * 10)
        if temp.get("solved"):
            hcs_pass8 += 1

        temp = hill_climbing_random_restart(nqueens8, maximum)
        if temp.get("solved"):
            hcrr_pass8 += 1

        temp = hill_climbing_instrumented(nqueens9)
        if temp.get("solved"):
            hci_pass9 += 1

        temp = hill_climbing_sideways(nqueens9, maximum * 10)
        if temp.get("solved"):
            hcs_pass9 += 1
        
        temp = hill_climbing_random_restart(nqueens9, maximum)
        if temp.get("solved"):
            hcrr_pass9 += 1
        
        temp = hill_climbing_instrumented(nqueens10)
        if temp.get("solved"):
            hci_pass10 += 1

        temp = hill_climbing_sideways(nqueens10, maximum)
        if temp.get("solved"):
            hcs_pass10 += 1

        temp = hill_climbing_random_restart(nqueens10, maximum * 10)
        if temp.get("solved"):
            hcrr_pass10 += 1

    print("8-queen hill climbing instrumented probability of success: %1.2f" % (hci_pass8/test_range))
    print("8-queen hill climbing sideways probability of success: %1.2f" % (hcs_pass8/test_range))
    print("8-queen hill climbing random restart probability of success: %1.2f" % (hcrr_pass8/test_range))
    print("9-queen hill climbing instrumented probability of success: %1.2f" % (hci_pass9/test_range))
    print("9-queen hill climbing sideways probability of success: %1.2f" % (hcs_pass9/test_range))
    print("9-queen hill climbing random restart probability of success: %1.2f" % (hcrr_pass9/test_range))
    print("10-queen hill climbing instrumented probability of success: %1.2f" % (hci_pass10/test_range))
    print("10-queen hill climbing sideways probability of success: %1.2f" % (hcs_pass10/test_range))
    print("10-queen hill climbing random restart probability of success: %1.2f" % (hcrr_pass10/test_range))
